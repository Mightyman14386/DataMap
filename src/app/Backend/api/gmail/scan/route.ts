...
export async function GET() {
	try {
		...
		console.log("[Gmail Scan] Looking up user:", session.user.email);

		// Look up user by email (matching how we save in auth.ts)
		const userDoc = await getDoc(doc(db, "users", session.user.email));
		const userData = userDoc.data();
		
		console.log("[Gmail Scan] User data found:", !!userData);
		
		if (!userData?.accessToken) {
			console.error("[Gmail Scan] No access token found for user");
		...
		console.log("[Gmail Scan] Starting email fetch...");

		// Fetch emails (limited to 100 for testing)
		...
		console.log(`[Gmail Scan] Found ${messageIds.length} total messages`);
		...
		console.log("[Gmail Scan] Fetching full message details...");
		...
		console.log(`[Gmail Scan] Found ${parsedEmails.length} relevant emails`);

		// Convert to services for analysis
		const discoveredServices = convertEmailsToServices(parsedEmails);
		console.log(`[Gmail Scan] Extracted ${discoveredServices.length} services from emails`);

		if (discoveredServices.length === 0) {
			console.log("[Gmail Scan] No services discovered");
			return NextResponse.json({ 
				count: 0, 
				summary: { red: 0, yellow: 0, green: 0 }, 
				results: [] 
		...
		console.log(`[Gmail Scan] Starting analysis pipeline for ${discoveredServices.length} services...`);
		console.log(`[Gmail Scan] Using userId: ${session.user.email}`);
		console.log(`[Gmail Scan] Analysis will timeout after 45 seconds`);
		...
		try {
			...
			console.warn("[Gmail Scan] Analysis timed out:", timeoutError);
		...
		console.log(`[Gmail Scan] Analysis complete: ${analysis.count} services analyzed`);
		...
		return NextResponse.json(
			{ error: "Failed to scan and analyze emails", details: String(error) },
			{ status: 500 }
		);
	}
}